...
class Player(pygame.sprite.Sprite):
    def __init__(self, game_mode):
        ...
        if self.game_mode == "Neuroevolution":
            ...
            layer_sizes = [2, 12, 2]  # TODO (Design your architecture here by changing the values)
            self.nn = NeuralNetwork(layer_sizes)

    def nn_input(self, screen_width, screen_height, obstacles, player_x, player_y):
        num = 1
        obstacle_y = []
        obstacle_x = []
        ...
        for i in range(num):
            if num_obstacles > 0:
                obstacle_x.append(obstacles[i]['x'])
                obstacle_y.append(obstacles[i]['y'])
                num_obstacles -= 1
        ...
        input = []
        for i in range(num):
            input.append((player_x - obstacle_x[i]) / screen_width)
            input.append((player_y - obstacle_y[i]) / screen_height)
    ...
    def think(self, screen_width, screen_height, obstacles, player_x, player_y):
        ...
        input = self.nn_input(screen_width, screen_height, obstacles, player_x, player_y)
